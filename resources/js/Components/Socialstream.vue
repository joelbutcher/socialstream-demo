    ...
    <div v-if="providers.length" class="space-y-6 mt-6 mb-2">
        ...
        <div class="grid gap-4">
            <a v-for="provider in providers" :key="provider.id"
               class="inline-flex items-center justify-center transition duration-200 w-full py-2.5 rounded-lg text-sm shadow-sm hover:shadow-md font-normal text-center"
               :class="{
                  'text-gray-700 dark:text-gray-300 border border-gray-400': provider.id !== 'apple',
                  'text-white dark:text-gray-300 border border-transparent bg-black hover:border-gray-300 dark:active:bg-gray-200': provider.id === 'apple',
               }"
               :href="route('oauth.redirect', provider.id)">
                <ProviderIcon :provider="provider" classes="h-6 w-6 mx-2"/>
                <span class="block font-medium text-sm">{{ provider.buttonLabel || provider.name }}</span>
            </a>
        </div>
    </div>
</template>
