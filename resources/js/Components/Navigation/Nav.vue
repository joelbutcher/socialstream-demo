...
const toggleMenu = () => {
  ...
  localStorage.setItem('nav_open', open.value);
};
</script>

<template>
  <div class="h-screen sticky top-0 bottom-4 z-30">
    <nav
        :class="clsx(
    ...
    >
      <div>
        ...
        <div
            :class="clsx(
                'flex items-center text-slate-900 dark:text-slate-200 py-5 px-6 ml-1',
            )"
        >
          <!-- Collapse Button -->
          <ChevronRightIcon
              :class="clsx('cursor-pointer w-6 h-6 transform transition delay-400', open && 'rotate-180')"
              @click="toggleMenu"
          />
        </div>
        ...
        <Link :href="route('dashboard')">
          <svg
              ...
              class="w-56 px-5 ml-1"
          >
            <path
                :class="clsx(
                    'fill-black dark:fill-slate-200',
                      'opacity-0 transition duration-400 ease',
                      open ? 'opacity-100 pointer-events-none' : 'pointer-events-auto',
                )"
