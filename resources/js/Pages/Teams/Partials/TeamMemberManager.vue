      ...
      <FormSection @submitted="addTeamMember">
        ...
        <template #form>
          <div class="col-span-6">
            <div class="max-w-xl text-sm text-slate-600 dark:text-slate-200">
          ...
          <div v-if="availableRoles.length > 0" class="col-span-6 lg:col-span-4">
            ...
            <div class="relative z-0 mt-1 border border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer">
              <button
                  ...
                  :class="{'border-t border-slate-200 dark:border-slate-700 rounded-t-none': i > 0, 'rounded-b-none': i != Object.keys(availableRoles).length - 1}"
                  @click="addTeamMemberForm.role = role.key"
              >
                <div :class="{'opacity-50': addTeamMemberForm.role && addTeamMemberForm.role != role.key}">
                  <!-- Role Name -->
                  <div class="flex items-center">
                    <div class="text-sm text-slate-800 dark:text-slate-200 font-semibold"
                  ...
                  <!-- Role Description -->
                  <div class="mt-2 text-xs text-slate-600 dark:text-slate-400 text-left">
                    {{ role.description }}
                  </div>
                </div>
              </button>
            ...
            <div v-for="invitation in team.team_invitations" :key="invitation.id"
              ...
              <div class="flex items-center">
                ...
                <PrimaryButton
                    ...
                    class="cursor-pointer ml-6 text-sm text-slate-500 focus:outline-none"
        ...
        <template #content>
          <div class="space-y-6">
            <div v-for="user in team.users" :key="user.id" class="flex items-center justify-between">
              <div class="flex items-center">
                <img class="w-8 h-8 rounded-full" :src="user.profile_photo_url" :alt="user.full_name">
                <div class="ml-4 text-slate-600 dark:text-slate-200">
                  {{ user.name }}
                </div>
              </div>
              ...
              <div class="flex items-center">
                ...
                <button
                    ...
                    class="ml-2 text-sm text-slate-400 dark:text-slate-600 underline"
                    @click="manageRole(user)"
                >
                  {{ displayableRole(user.membership.role) }}
                </button>

                <div v-else-if="availableRoles.length" class="ml-2 text-sm text-slate-400">
                  {{ displayableRole(user.membership.role) }}
                </div>
          ...
          <div class="relative z-0 mt-1 border border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer">
            ...
            >
              <div :class="{'opacity-50': updateRoleForm.role && updateRoleForm.role !== role.key}">
                <!-- Role Name -->
                <div class="flex items-center">
                  <div class="text-sm text-slate-800 dark:text-slate-200" :class="{'font-semibold': updateRoleForm.role === role.key}">
                    {{ role.name }}
                  </div>
